...
Menu bar utility to remap mouse scroll and buttons to keyboard navigation actions.

## Run in Xcode (recommended)

Accessibility permission only works reliably when the app has a **bundle identifier**.

1. Open **`ThreeBlindMice.xcodeproj`** (not `Package.swift`).
...
6. Menu bar should show **Remapping engine: running**.
